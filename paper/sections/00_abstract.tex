...
对问题四和问题五，由于数据集中没有真实运动型/非运动型标签，也没有静止性震颤、运动迟缓、肌肉僵直、疼痛、痴呆、睡眠障碍六类临床症状标签，本文仅在 Dataset 1 的 188 名 PD 受试者内部进行无监督潜在语音表型分析。第 5b 阶段无奖励权重敏感性分析显示，二类分型原方案 \feature{top20_biomarkers} / \feature{pca_5} / KMeans 的 top-1 频率为 37.3\%，因此保留为相对推荐候选方案，并同步报告多个可接受候选。第 6b 阶段显示，六类潜在语音表型方案 \feature{top20_biomarkers} / \feature{pca_5} / AgglomerativeClustering 的 top-1 频率为 87.1\%，可作为 \(k=6\) 潜在语音表型聚类的稳健主推荐方案。上述分型结果均不应解释为已经由临床标签确认的真实亚型或症状类别。
